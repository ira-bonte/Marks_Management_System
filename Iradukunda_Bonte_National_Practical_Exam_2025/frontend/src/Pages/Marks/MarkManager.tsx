    ...
    return (
        <div className="max-w-2xl mx-auto py-6">
            <form onSubmit={handleSubmit} className="space-y-4 mb-6 bg-gray-50 p-4 rounded shadow">
                ...
                <input type="number" name="Formative_Ass" value={form.Formative_Ass} onChange={handleChange} className="border w-full px-4 py-2" placeholder="Formative Assessment (0-100)" required />
                <input type="number" name="Summative_Ass" value={form.Summative_Ass} onChange={handleChange} className="border w-full px-4 py-2" placeholder="Summative Assessment (0-100)" required />
                <input type="number" name="Comprehensive_Ass" value={form.Comprehensive_Ass} onChange={handleChange} className="border w-full px-4 py-2" placeholder="Comprehensive Assessment (0-100)" required />

                <div className="text-right text-sm text-gray-600">
                    <span className="font-semibold">Total (Average):</span> <span className="font-bold text-black">{form.Total_Marks_100} / 100</span>
                </div>
            ...
            <div className="space-y-3">
                {marks.map((m) => (
                    <div key={m.Mark_Id} className="bg-gray-100 border p-4 rounded shadow-sm">
                        <div className="mb-2">
                            <p><strong>Trainee ID:</strong> {m.Trainee_Id}</p>
                            <p><strong>Module ID:</strong> {m.Module_Id}</p>
                            <p className="text-sm text-gray-700">
                                <strong>Formative:</strong> {m.Formative_Ass} &nbsp;|&nbsp;
                                <strong>Summative:</strong> {m.Summative_Ass} &nbsp;|&nbsp;
                                <strong>Comprehensive:</strong> {m.Comprehensive_Ass}
                            </p>
                            <p className="mt-1"><strong className="text-black">Total Average:</strong> <span className="font-bold text-blue-700">{m.Total_Marks_100} / 100</span></p>
                        </div>
                        <div className="space-x-2">
                            <button onClick={() => handleEdit(m)} className="bg-yellow-500 text-white px-3 py-1 rounded">Edit</button>
                            <button onClick={() => handleDelete(m.Mark_Id)} className="bg-red-600 text-white px-3 py-1 rounded">Delete</button>
                        </div>
                    </div>
                ))}
            </div>
